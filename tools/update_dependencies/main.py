...
if __name__ == "__main__":
    ...
    for tool in {'autoconf', 'automake', 'git', 'libtool', 'libxcrypt', 'sqlite_autoconf'}:  # , 'patchelf'
        update_result = _others.get_update_map(tool, ENV_PATH)
        if update_result:
            _others.update(tool, ENV_PATH, update_result)

    # git ls-remote --refs --tags https://github.com/openssl/openssl.git 'OpenSSL_1_1_1[a-z]'
    # git ls-remote --refs --tags https://github.com/mackyle/sqlite.git 'version-3.*'
